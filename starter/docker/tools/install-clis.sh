...
set -eu
export GONOSUMDB='*'
export GOPRIVATE='github.com/go-sum/*'

# ── Assets CLI ────────────────────────────────────────────────────────────────
if [ -n "${ASSETS_CLI_VERSION:-}" ]; then
  echo "installing assets CLI v${ASSETS_CLI_VERSION}..."
  go install "github.com/go-sum/assets/cli@v${ASSETS_CLI_VERSION}"
...
if [ -n "${DOCS_CLI_VERSION:-}" ]; then
  ...
  go install "github.com/go-sum/docs/cli@v${DOCS_CLI_VERSION}"
