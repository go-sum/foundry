...
func newFakeSessionBackend() *fakeSessionBackend {
	return &fakeSessionBackend{entries: make(map[string]fakeKVEntry)}
}

func (b *fakeSessionBackend) Delete(_ context.Context, keys ...string) error {
...
func (b *fakeSessionBackend) SessionRead(_ context.Context, key string, now time.Time) ([]byte, int64, bool, error) {
	b.readCalls++
	if b.readErr != nil {
...
func (b *fakeSessionBackend) SessionSave(_ context.Context, key string, data []byte, absolute time.Time, idleTTL time.Duration, version int64, now time.Time) (int64, bool, bool, error) {
	if b.alwaysConflict {
...
func TestKVStore_ReadWrite(t *testing.T) {
	...
	if _, ok := backend.entries["session:"+token]; !ok {
		t.Fatalf("expected namespaced key %q in backend", "session:"+token)
	}
...
func TestKVStore_CustomPrefix(t *testing.T) {
	backend := newFakeSessionBackend()
	store := NewKVStore(backend, KVStoreConfig{Prefix: "app-a:session:"})
	...
	if _, ok := backend.entries["app-a:session:"+token]; !ok {
		t.Fatalf("expected custom-prefixed key %q in backend", "app-a:session:"+token)
	}
}
...
func TestKVStore_NotFound(t *testing.T) {
	store := NewKVStore(newFakeSessionBackend())
...
func TestKVStore_InvalidTokenRejectedBeforeBackendRead(t *testing.T) {
	backend := newFakeSessionBackend()
	store := NewKVStore(backend)
	...
	if backend.readCalls != 0 {
		t.Fatalf("backend readCalls = %d, want 0 (backend must not be reached for invalid tokens)", backend.readCalls)
	}
}

func TestKVStore_ReadError(t *testing.T) {
	store := NewKVStore(&fakeSessionBackend{readErr: context.DeadlineExceeded, entries: make(map[string]fakeKVEntry)})
...
func TestKVStore_SaveError(t *testing.T) {
	...
	store := NewKVStore(&fakeSessionBackend{saveErr: want, entries: make(map[string]fakeKVEntry)})
...
func TestKVStore_TokenCollisionExhaustion(t *testing.T) {
	backend := &fakeSessionBackend{entries: make(map[string]fakeKVEntry), alwaysConflict: true}
	store := NewKVStore(backend)
