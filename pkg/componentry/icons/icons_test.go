...
func TestDefault_packageLevel(t *testing.T) {
	// Use unique keys to avoid interfering with other tests sharing the Default registry.
	const testKey icons.Key = "test-package-level-key"
	ref := icons.Ref{Sprite: "/pkg.svg", ID: "pkg-icon"}

	icons.Register(testKey, ref)

	got, ok := icons.Resolve(testKey)
	if !ok {
		t.Fatalf("package-level Resolve: got ok=false")
	}
	if got != ref {
		t.Errorf("package-level Register/Resolve: got %+v, want %+v", got, ref)
	}
}

func TestDefault_RegisterSet_packageLevel(t *testing.T) {
	const keyA icons.Key = "test-set-a"
	const keyB icons.Key = "test-set-b"
	set := map[icons.Key]icons.Ref{
		keyA: {Sprite: "/set.svg", ID: "set-a"},
		keyB: {Sprite: "/set.svg", ID: "set-b"},
	}

	icons.RegisterSet(set)

	for key, want := range set {
		got, ok := icons.Resolve(key)
		if !ok {
			t.Errorf("package-level RegisterSet: key %q not found", key)
			continue
		}
		if got != want {
			t.Errorf("package-level RegisterSet key %q: got %+v, want %+v", key, got, want)
		}
	}
}
